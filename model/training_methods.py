...
def _discriminator_loss(discr_output_real_vis, discr_output_fake_vis):
    ...
    Returns:
        discr_loss: total loss of discriminator
    """
...
def _generator_loss(discr_output_fake_vis, fake_vis_batch, real_vis_batch):
    ...
    Returns:
        ...
        fooling_discr_loss: loss corresponding to how well the generator tricked the discriminator
        real_vis_similarity_loss: loss corresponding to how well the generator recreated the real VIS images
    """
...
def train_step(ir_batch, real_vis_batch, generator, discriminator, gen_optimizer, discr_optimizer):
    ...
    Returns:
        ...
        total_gen_loss: total generator loss
        fooling_discr_loss: part of generator loss on how well it fooled the discriminator
        real_vis_similarity_loss: part of generator loss on how similar the fake image is to the real one
    """
...
def eval_model(test_dataset, generator, discriminator):
    ...
    Args:
        ...
        generator (tf.Model): instance of generator model
        discriminator (tf.Model): instance of discriminator model
    Returns:
        discr_loss: discriminator loss
        total_gen_loss: total generator loss
        fooling_discr_loss: part of generator loss on how well it fooled the discriminator
...
def generate_example_images(dataset, gen_model):
    """selects a random image triplet from dataset
    Args:
        dataset (tf.Data.Dataset): test or training dataset
        gen_model (tf.Model): instance of generator model
    Returns:
        ir_img (tf.tensor): randomly picked infra-red image
        predict_vis_img (tf.tensor): corresponding fake visible image generated by the generator
        vis_img (tf.tensor): corresponding real visible image
    """
...
def pick_image_pair_from_dataset(dataset):
    """selects a random image pair from dataset
    Args:
        dataset (tf.Data.Dataset): test or training dataset
    Returns:
        ir_img (tf.tensor): randomly picked infra-red image
